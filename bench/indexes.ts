...
const insertStmtNoIdx = dbNoIdx.prepare(
  "INSERT INTO users (name, email, role, score, created_at) VALUES (?, ?, ?, ?, ?)",
);
const insertStmtIdx = dbIdx.prepare(
  "INSERT INTO users (name, email, role, score, created_at) VALUES (?, ?, ?, ?, ?)",
);
...
{
  const stmt = dbNoIdx.query(
    "SELECT * FROM users WHERE role = ? AND score > ? LIMIT 50",
  );
  bench("No index", () => {
    stmt.all(
...

{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `No indexes:    ${INSERT_COUNT} inserts in ${elapsed.toFixed(1)}ms  (${((elapsed / INSERT_COUNT) * 1000).toFixed(1)}µs/insert)`,
  );
}

{
  const stmt = dbIdx.prepare(
    "INSERT INTO users (name, email, role, score, created_at) VALUES (?, ?, ?, ?, ?)",
  );
  const start = Bun.nanoseconds();
  for (let i = 0; i < INSERT_COUNT; i++) {
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `With 4 indexes: ${INSERT_COUNT} inserts in ${elapsed.toFixed(1)}ms  (${((elapsed / INSERT_COUNT) * 1000).toFixed(1)}µs/insert)`,
  );
}
...
console.log(
  "For read-heavy workloads (typical ORM usage), indexes are essential.",
);
console.log(
  "sqlite-zod-orm auto-creates them from the { indexes } config option.",
);
