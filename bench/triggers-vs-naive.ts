...

{
  ...
  console.log(
    `With triggers:    ${ROWS} inserts in ${elapsed.toFixed(1)}ms  (${((elapsed / ROWS) * 1000).toFixed(1)}µs/insert)`,
  );
}

{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `Without triggers: ${ROWS} inserts in ${elapsed.toFixed(1)}ms  (${((elapsed / ROWS) * 1000).toFixed(1)}µs/insert)`,
  );
}
...

{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `With triggers:    ${ROWS} updates in ${elapsed.toFixed(1)}ms  (${((elapsed / ROWS) * 1000).toFixed(1)}µs/update)`,
  );
}

{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `Without triggers: ${ROWS} updates in ${elapsed.toFixed(1)}ms  (${((elapsed / ROWS) * 1000).toFixed(1)}µs/update)`,
  );
}
...
const watermark =
  (dbTrigger.query("SELECT MAX(id) as m FROM _changes").get() as any)?.m ?? 0;
...
// A) Trigger-based: MAX(id) check
{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `A) Trigger MAX(id):      ${elapsed.toFixed(1)}ms  (${((Bun.nanoseconds() - start) / POLL_ITERATIONS).toFixed(0)}ns/poll)`,
  );
}

// B) PRAGMA data_version
{
  let lastVersion = (dbNaive.query("PRAGMA data_version").get() as any)
    .data_version;
  const start = Bun.nanoseconds();
  for (let i = 0; i < POLL_ITERATIONS; i++) {
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `B) PRAGMA data_version:  ${elapsed.toFixed(1)}ms  (${((Bun.nanoseconds() - start) / POLL_ITERATIONS).toFixed(0)}ns/poll)`,
  );
}
...
// C) COUNT + MAX fingerprint
{
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `C) COUNT+MAX fingerprint: ${elapsed.toFixed(1)}ms  (${((Bun.nanoseconds() - start) / POLL_ITERATIONS).toFixed(0)}ns/poll)`,
  );
}
...
{
  let wm = 0;
  const stmtInsert = dbTrigger.prepare(
    "INSERT INTO users (name, score) VALUES (?, ?)",
  );
  const stmtMax = dbTrigger.query("SELECT MAX(id) as m FROM _changes");
  const stmtChanges = dbTrigger.query(
    "SELECT id, row_id FROM _changes WHERE id > ? ORDER BY id",
  );
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `A) Triggers:             ${elapsed.toFixed(1)}ms  (${((elapsed / ROUNDTRIP) * 1000).toFixed(1)}µs/roundtrip)`,
  );
}
...
// B) data_version + full re-query
{
  let lastVersion = (dbNaive.query("PRAGMA data_version").get() as any)
    .data_version;
  const stmtInsert = dbNaive.prepare(
    "INSERT INTO users (name, score) VALUES (?, ?)",
  );
  ...
  const elapsed = (Bun.nanoseconds() - start) / 1_000_000;
  console.log(
    `B) data_version + fetch: ${elapsed.toFixed(1)}ms  (${((elapsed / ROUNDTRIP) * 1000).toFixed(1)}µs/roundtrip)`,
  );
}
...
console.log(
  "In return you get row-level, operation-level, table-level granularity.",
);
...
console.log(
  "For apps without listeners, use { reactive: false } to pay zero cost.",
);
