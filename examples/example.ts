...
console.log(
  "Top scorers:",
  topScorers.map((u: any) => `${u.name}: ${u.score}`),
);

// $or — find admins OR high scorers
const adminsOrHighScore = db.users
  .select()
  .where({ $or: [{ role: "admin" }, { score: { $gt: 50 } }] })
  .all();
console.log(
  "Admins or high scorers:",
  adminsOrHighScore.map((u: any) => `${u.name} (${u.role}, ${u.score})`),
);
...
const dostoevsky = db.authors.insert({
  name: "Fyodor Dostoevsky",
  country: "Russia",
});
const kafka = db.authors.insert({
  name: "Franz Kafka",
...
console.log(
  `Seeded ${db.authors.select().count()} authors, ${db.books.select().count()} books`,
);
...
console.log(
  "Tolstoy books:",
  tolstoyBooks.map((b: any) => b.title),
);
...
console.log(
  `"${warAndPeace.title}" by ${bookAuthor.name} (${bookAuthor.country})`,
);

// one-to-many: author → books
const dostoevskyBooks = dostoevsky.books();
console.log(
  `Dostoevsky's books: ${dostoevskyBooks.map((b) => b.title).join(", ")}`,
);
...
const booksWithAuthors = db.books
...
for (const row of booksWithAuthors) {
  console.log(
    `  ${(row as any).year} - ${(row as any).title} by ${(row as any).authors_name} (${(row as any).authors_country})`,
  );
}

const longBooks = db.books
  .select("title", "pages")
...
console.log(
  "Long books:",
  longBooks.map((b: any) => `${b.title} (${b.pages}p) by ${b.authors_name}`),
);
...
const tolstoyBooksByRef = db.books
  .select("title", "year")
  .where({ author: tolstoy } as any)
  .orderBy("year", "asc")
  .all();
console.log(
  `Tolstoy's books (entity ref):`,
  tolstoyBooksByRef.map((b: any) => `${b.title} (${b.year})`),
);
...
for (const row of russianBooks) {
  console.log(
    `  ${(row as any).year} - ${(row as any).book} by ${(row as any).author}`,
  );
}
...
db.users.upsert(
  { email: "bob@example.com" },
  { name: "Bob", email: "bob@example.com", role: "moderator", score: 80 },
);
console.log(
  "Bob after upsert:",
  db.users.select().where({ email: "bob@example.com" }).get()?.role,
);
...
console.log(
  "After transaction — Alice score:",
  db.users.select().where({ name: "Alice" }).get()?.score,
);
console.log(
  "After transaction — Eve exists:",
  !!db.users.select().where({ name: "Eve" }).get(),
);
