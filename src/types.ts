...
export const asZodObject = (s: z.ZodType<any>) =>
...
export function defineView<S extends z.ZodType<any>>(
  schema: S,
  as: string,
): ViewDefinition<S> {
  return { schema, as };
...
> = Table extends keyof R
  ? {
      readonly [
        FK in Extract<keyof R[Table], string> as StripIdSuffix<FK>
...
> = {
  readonly [
    ChildTable in Extract<keyof R & keyof S, string> as _FKTargetsContain<
      R[ChildTable],
      ParentTable
    > extends true
      ? ChildTable
...
> = {
  ...
  findOrCreate: (
    conditions: Partial<z.infer<S[Table & keyof S]>>,
    defaults?: Partial<z.infer<S[Table & keyof S]>>,
  ) => { entity: NavEntity<S, R, Table>; created: boolean };
  delete: ((id: number) => void) &
    (() => DeleteBuilder<NavEntity<S, R, Table>>);
  restore: (id: number) => void;
  select: {
    (): QueryBuilder<NavEntity<S, R, Table>>;
    <K extends (keyof z.infer<S[Table & keyof S]> | "id") & string>(
      ...cols: K[]
    ): QueryBuilder<NavEntity<S, R, Table>, Pick<NavEntity<S, R, Table>, K>>;
  };
  count: () => number;
  on: ((
    event: "insert" | "update",
    callback: (row: NavEntity<S, R, Table>) => void | Promise<void>,
  ) => () => void) &
    ((
      event: "delete",
      callback: (row: { id: number }) => void | Promise<void>,
    ) => () => void);
  _tableName: string;
...
export type EntityAccessor<S extends z.ZodType<any>> = {
  ...
  ) => { entity: AugmentedEntity<S>; created: boolean };
  ...
  select: {
    ...
    <K extends (keyof InferSchema<S> | "id") & string>(
      ...cols: K[]
    ): QueryBuilder<AugmentedEntity<S>, Pick<AugmentedEntity<S>, K>>;
  };
  count: () => number;
  on: ((
    event: "insert" | "update",
    callback: (row: AugmentedEntity<S>) => void | Promise<void>,
  ) => () => void) &
    ((
      event: "delete",
      callback: (row: { id: number }) => void | Promise<void>,
...
export type ReadonlyEntityAccessor<S extends z.ZodType<any>> = {
  select: {
    ...
    <K extends keyof InferSchema<S> & string>(
      ...cols: K[]
    ): QueryBuilder<ReadonlyEntity<S>, Pick<ReadonlyEntity<S>, K>>;
  };
  count: () => number;
  _tableName: string;
  readonly _schema?: S;
